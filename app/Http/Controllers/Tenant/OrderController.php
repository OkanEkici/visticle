...
use App\Tenant\Order,App\Tenant\Order_Attribute,App\Tenant\Order_Status;
use App\Tenant\OrderArticle,App\Tenant\OrderArticle_Attribute,App\Tenant\OrderArticle_Status;
use App\Tenant\OrderVoucher,App\Tenant\OrderVoucherArticle,App\Tenant\OrderVoucherCategory;
use App\Tenant\Order_Document, App\Tenant\Order_Document_Type, App\Tenant\Order_Document_Attribute;
use App\Tenant\Setting,App\Tenant\Settings_Attribute;
use App\Tenant\Payment,App\Tenant\Invoice;
use App\Tenant\Config_Payment, App\Tenant\Config_Shipment;
use App\Tenant\Article_Variation;
use Redirect,Response;
use PDF,Validator, Session, Auth, Carbon\Carbon;
use App\Mail\OrderConfirmed, App\Mail\OrderConfirmed_industry;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Log;
use App\Tenant, App\Tenant_Keys;
use App\Tenant\Customer;
use App\Http\Controllers\Tenant\Providers\Zalando\ZalandoController;
use App\Mail\NotificationTrackingID;
use App\Manager\Plattform\PlattformManager;
use App\Tenant\Branch;
use App\Tenant\Provider;
use App\Tenant\InvoiceCorrection;
...
class OrderController extends Controller
{
    ...
    public function show($id)
    {
        ...
        foreach($order->articles()->get() as $orderArticle)
        {
            ...
            $orderedArticlesContent_retour[$articleCount]['position'] = $orderedArticlesContent[$articleCount]['position'];
            $orderedArticlesContent_retour[$articleCount]['name'] = $orderedArticlesContent[$articleCount]['name'];
            ...
            $orderedArticlesContent_retour[$articleCount]['ean'] = $orderedArticlesContent[$articleCount]['ean'];
            $orderedArticlesContent_retour[$articleCount]['quantity'] = '<input value="'.(($orderedArticlesContent[$articleCount]['quantity']-$orderArticle->returned <= 0)? 0 : $orderedArticlesContent[$articleCount]['quantity']-$orderArticle->returned).'" name="quantity['.$orderArticle->id.']" id="tf_quantity_'.$orderArticle->id.'" type="number"  class="form-control border px-3 border-1 " min="0">';
            $retour_quantity_val[$articleCount]['quantity_val'] = (($orderedArticlesContent[$articleCount]['quantity']-$orderArticle->returned <= 0)? 0 : $orderedArticlesContent[$articleCount]['quantity']-$orderArticle->returned);
        ...
        return view('tenant.modules.order.show.order', [
            ...
            'orderedArticlesContent_retour' => $orderedArticlesContent_retour,
            'retour_quantity_val' => $retour_quantity_val,
    ...
    public function addTrackingID(Request $request,$order_id){
        $order=Order::find($order_id);

        $email=$order->getShipmentAddressByKey('email');
        $tracking_id=$request->get('tracking_id');

        if($order_id && $email && !empty($tracking_id) && $order->provider->type->provider_key=='check24'){
            $order->updateOrCreateAttribute('check24_tracking_id',$tracking_id);

            //Email abschicken!!!
            $email=$order->getShipmentAddressByKey('email');
            Mail::to($email)->send(new NotificationTrackingID($order));
        }

        return redirect()->to(route('tenant.orders.show',['identifier'=>config('tenant.identifier'),'id'=>$order_id]));
    }
    ...
    public function updatePayment(Request $request, $id) {
       ...
       $Tenant_type = config()->get('tenant.tenant_type');
       if($Tenant_type=='vstcl-industry'){ $totalPrice = ($order->getFullPrice_ind() / 100 ); }
       else{ $totalPrice = ($order->getFullPrice() / 100 ); } // - (float)(($order->voucher_price)? $order->voucher_price : 0)
            ...
            if(!file_exists($pdfPath.$pdfName)) {
                switch($docType) {
                    case 'invoice':
                        ...
                        $pdf = $this->pdfRechnung($orderId, $invoice); break;
    ...
    public function downloadDocument(Request $request, $orderId, $docType, $file = false) {

        $pdfPath = storage_path()."/customers/".$request->attributes->get('identifier')."/pdf/";
        if($docType == 'db_docs' && $file){return response()->download($pdfPath.$file);}
        $thisTenant = config()->get('tenant');
        $pdf = null; $pdfName = '';
        $order = Order::find($orderId);
        ...
        if(!file_exists($pdfPath.$pdfName)) {
            switch($docType) {
                case 'invoice':
                    ...
                    $pdf = $this->pdfRechnung($orderId, $invoice);
                    break;
                case 'retoure': $pdf = $this->pdfRetoure($orderId); break;
                case 'delivery_note': $pdf = $this->pdfLieferschein($orderId); break;
                case 'order_confirmation': $pdf = $this->pdfBestellbestaetigung($orderId, $AB_Nummer, $thisTenant['identifier']); break;
    ...
    public function pdfRechnungKorrektur($id,$Doc_BelegNr=false) {
        $order = Order::where('id','=', $id)->first();
        $invoice = $order->invoices()->first();
        if(!$invoice) { return false; }
        $companyAddr = $this->getCompanyAddr();
        ...
        if($Doc_BelegNr)
		{	$checkDocumentExist = Order_Document_Attribute::where('name', '=', 'beleg-nr')->where('value', '=', $Doc_BelegNr)->first();
			if($checkDocumentExist)
			{	$Doc_Datas = Order_Document_Attribute::where('name', '=', 'return-data')->where('fk_order_document_id', '=', $checkDocumentExist->fk_order_document_id)->first();
				if($Doc_Datas){$Doc_Datas = json_decode($Doc_Datas->value,true);}else{$Doc_Datas=[];}
			}else{$Doc_Datas=[];}
		}else{$Doc_Datas = [];}
        $eans = [];foreach($Doc_Datas as $ean => $quantity){$eans[]=$ean; }
        $articles = $order->articles()->get();
        foreach($articles as $article)
        {
            $mainArticle = $article->article()->first();
            ...
            $data['positions']['rows'][] = [
                'values' => [
                    ...
                    $Doc_Datas[$org_ean],
                    Article::getFormattedPrice($article->price / 100),
                    Article::getFormattedPrice($article->price * $Doc_Datas[$org_ean] / 100)
                ]
            ];
        }
        ...
        $pdf = PDF::loadView('tenant.pdf.rechnungskorrektur', [
            ...
            'payment' => $payment,
            'number' => $Doc_BelegNr,
            'RE_number' => Setting::getReceiptNameWithNumberByKey('invoice', $invoice->number)
            ])
            ->setOption('margin-bottom', '30mm')
            ->setOption('footer-html', $footerHtml);
        return $pdf;
    ...
    public function addRetour($id,$data = [])
    {   $pdfPath = storage_path()."/customers/".config()->get('tenant.identifier')."/pdf/";
        $order = Order::where('id','=', $id)->first();

        $invoice = $order->invoices()->first();
        if(!$invoice) { return false; }
        $OrderArticle_Status_teilwRetourniert_id = OrderArticle_Status::where('description', '=', 'teilweise Retourniert')->first()->id;
        $OrderArticle_Status_Retourniert_id = OrderArticle_Status::where('description', '=', 'Retourniert')->first()->id;
        $allArticleStatusRetourned = false; $untrackStatusRetourned = false;
        foreach($data as $ean => $quantity)
        {
            foreach($order->articles()->get() as $orderArticle)
            {
                $article = $orderArticle->article()->first();
                $variation = $orderArticle->variation()->first();
                if($ean == (($variation) ? $variation->getEan().(($variation->extra_ean!="")? ", ".$variation->extra_ean:'') : $article->ean))
                {   //$orderArticle->fk_orderarticle_status_id = 6;$orderArticle->save();
                    if($quantity > ($orderArticle->quantity - $orderArticle->returned)){$quantity = $orderArticle->quantity;}
                    $orderArticle->returned = $orderArticle->returned + $quantity;
                    if($quantity>0)
                    {
                        //*/ Export Fee Umsatzdatei Add Line
                        $org_ean = $variation->getEan();
                        $price = ($orderArticle->price / (($quantity==0)?1:$quantity) );
                        $OrderNumber = "GS-".str_pad($order->number, 6, '0', STR_PAD_LEFT);
                        ...
                        { $OrderNumber = "GS-".str_pad($order->number, strlen((string) $OrderNumberType->value), '0', STR_PAD_LEFT); }
                        ...
                        $zaC->fee_export($variation, $OrderNumber, 'R',$VerkaufsFilialie,$UrsprungsFilialie,$org_ean, $price, date("Ymd"), date("H:i:s"),'vsshop');
                        //*/

                        if($orderArticle->returned < $orderArticle->quantity){$orderArticle->fk_orderarticle_status_id = $OrderArticle_Status_teilwRetourniert_id;}
                        else{$orderArticle->fk_orderarticle_status_id = $OrderArticle_Status_Retourniert_id;}
                        $orderArticle->save();
                    }
        ...
        foreach($order->articles()->get() as $orderArticle)
        {if(($orderArticle->fk_orderarticle_status_id != $OrderArticle_Status_Retourniert_id)){$untrackStatusRetourned = true;} }
        if(!$untrackStatusRetourned)
        {   $allArticleStatusRetourned=true;
            $OrderArticle_Status_Retourniert_id = Order_Status::where('key', '=', 'returned')->first()->id;
            $order->fk_order_status_id = $OrderArticle_Status_Retourniert_id; $order->save();
        }else
        {   $OrderArticle_Status_teilwRetourniert_id = Order_Status::where('key', '=', 'partially_returned')->first()->id;
            $order->fk_order_status_id = $OrderArticle_Status_teilwRetourniert_id; $order->save();
        }

        $invoiceCorrectionCount = InvoiceCorrection::count();
        $invoiceCorrection = InvoiceCorrection::create([ 'fk_order_id' => $order->id, 'number' => ++$invoiceCorrectionCount ]);

        $Doc_BelegNr = Setting::getReceiptNameWithNumberByKey('invoice_correction', $invoiceCorrection->number);
        $pdfName = $Doc_BelegNr;

        $suffix = "_".date("Ymd").".pdf";
        if(file_exists($pdfPath.$pdfName.$suffix)){$suffix = "_".date("Ymd_His").".pdf"; }


        $newDocument = Order_Document::updateOrCreate(
        [ 'fk_order_id' => $order->id, 'filename' => $pdfName.$suffix, 'fk_order_document_type_id' => Order_Document_Type::where('name', '=', 'rechnungskorrektur')->first()->id ] );
        $newDocument_attribute = Order_Document_Attribute::updateOrCreate([ 'name' => 'beleg-nr', 'value' => $Doc_BelegNr,'fk_order_document_id' => $newDocument->id ] );
        $newDocument_data_attribute = Order_Document_Attribute::updateOrCreate([ 'name' => 'return-data', 'value' => json_encode($data),'fk_order_document_id' => $newDocument->id ] );


        $pdf = $this->pdfRechnungKorrektur($id,$Doc_BelegNr);
        if(file_exists($pdfPath.$pdfName.$suffix)){
            $old = getcwd();
            chdir($path_to_file);
            unlink($pdfPath.$pdfName.$suffix);
            chdir($old);
        }
        $pdf->save($pdfPath.$pdfName.$suffix);


        return true;
    }

    public function singleRetour(Request $request, $id, $ean, $quantity)
    {   // Artikelstatus auf Retour setzen und Auftragsstatus ändern
        $order = Order::where('id','=', $id)->first();
        $data = []; $data[$ean] = $quantity;
        if($this->addRetour($id,$data))
        {return redirect()->back()->withSuccess('Eine Retour wurde erfolgreich erzeugt!');}
        else{return redirect()->back()->withError('Fehler!');}
    }

    public function updateRetour(Request $request, $id)
    {   $order = Order::where('id','=', $id)->first();
        $data = (array)($request->quantity);
        $new_data=[];
        if(($data))
        {   foreach($data as $orderArticleID => $RetourQuantity)
            {
                $orderArticle = $order->articles()->where('id','=', $orderArticleID)->first();
                $variation = $orderArticle->variation()->first();
                $org_ean = $variation->getEan();
                $new_data[$org_ean]=$RetourQuantity;
            }

            if($this->addRetour($id,$new_data))
            {return redirect()->back()->withSuccess('Eine Retour wurde erfolgreich erzeugt!');}
            else{return redirect()->back()->withError('Fehler!');}
        }else{ return redirect()->back()->withError('Fehler! Keine Daten empfangen!'); }
    }


    protected function pdfLieferschein_check24($id){
        $order = Order::where('id','=', $id)->first();
        ...
        foreach($articles as $article) {
            ...
            $data['positions'][] = [
                ...
                'packed' => $article->packed,
                'price' =>  Article::getFormattedPrice($article->price / 100),
                'price_total' =>  Article::getFormattedPrice($article->price  / 100),
        ...
        $view=$settings['receipts']['delivery']['view'];



        $footerHtml = view()->make('tenant.pdf.provider.check24.footer',['companyAddr' => $companyAddr])->render();
        $pdf = PDF::loadView($view, [
            'Tenant' => config()->get('tenant.identifier'),
            'data' => $data,
            'order' => $order,
            'shippingAddr' => $shippingAddr,
            'orderNumber' => $order->attributes()->where('name','check24_order_id')->first()->value,
            'gender' => $order->getShipmentAddressByKey('gender'),
            'orderDate' => date('d.m.Y', strtotime($order->created_at)),
            'companyAddr' => $companyAddr,
            'number' => Setting::getReceiptNameWithNumberByKey('delivery_note', $order->number),
            'price' => number_format($order->getPriceWithTaxes(),2),
            'receipt_type' => 'lieferschein',
            ])
            ->setOption('margin-bottom', '50mm')
            ->setOption('footer-html', $footerHtml);
        return $pdf;
    }
    public function pdfLieferschein($id){
        ...
        if($provider->type->provider_key=='check24')
        {
            return $this->pdfLieferschein_check24($id);
        }
        ...
        $pdf = PDF::loadView('tenant.pdf.lieferschein', [
            ...
            'order' => $order,
            'shippingAddr' => $shippingAddr,
            'orderNumber' => Setting::getReceiptNameWithNumberByKey('delivery_note', $order->number),
            'companyAddr' => $companyAddr,
            'number' => Setting::getReceiptNameWithNumberByKey('delivery_note', $order->number)
            ])
            ->setOption('margin-bottom', '30mm')
    ...
    protected function pdfPachschein_check24($id){
        ...
        foreach($articles as $article) {
            ...
            $data['positions'][] = [
                    ...
                    'img' => (($varArticle->getThumbnailBigImg()->first()) ? Storage::disk('public')->url($varArticle->getThumbnailBigImg()->first()->location) : '') ,
                    'color' => $varArticle->getColorText(),
                    'size' => $varArticle->getSizeText(),
                    'length' => $varArticle->getSizeText(),
    ...
    public function pdfRetoure_check24($id){
        $order = Order::where('id','=', $id)->first();

        $companyAddr = $this->getCompanyAddr(); $billingAddr = $this->getBillingAddr($order);
        ...
        $settings=$plattform_manager->getPlattformSettings($order->provider,['receipts'=>true]);
        $subdomain=config()->get('tenant.identifier');
        $view=$settings['receipts']['retoure']['view'];
        $retoure_settings=$settings['receipts']['retoure'];


        //Jetzt müssen wir noch den Text für die Retoure bestimmen!
        $view_text='';
        $toal_price=number_format($article->price  / 100,2);
        if(isset($retoure_settings['option']) && isset($retoure_settings['option']['price']) && isset($retoure_settings['option']['text']) ){
            if($toal_price >= $retoure_settings['option']['price']){
                $view_text=$retoure_settings['option']['text'];

            }
            else{
                $view_text=$retoure_settings['text'];
    ...
    public function pdfRetoure($id){
        ...
        if($provider->type->provider_key=='check24')
        {
            return $this->pdfRetoure_check24($id);
        }
    ...
    protected function getProcessedOrderDocuments(Order $order)
    {
        ...
        {   $orderDocs = [
            ...
            if(!$without_invoice){
                $orderDocs['invoice']=['title' => 'Rechnung'];
            }
        }
        else{
            $orderDocs = [
        ...
        $tableRows = []; $showLinks = ['order_confirmation','invoice','packing'/*,'retoure'*/];
         $pdfPath = storage_path()."/customers/".config()->get('tenant.identifier')."/pdf/";
        foreach($orderDocs as $key => $doc) {
            ...
            $tableRows[] =
            [   ($order->isFulfilled() || $order->isInReturn() || $order->isReturned()) ? '<a href="'.route('tenant.orders.pdf.downloadDocument', [config('tenant.identifier'), $order->id, $key,false]).'">'.$name.'</a>'
            : ((in_array($key, $showLinks) && $name != "" && file_exists($pdfPath.$pdfName) )? '<a href="'.route('tenant.orders.pdf.downloadDocument', [config('tenant.identifier'), $order->id, $key,false]).'">'.$name.'</a>'
            : '' )
            , $doc['title'] ];
        }

        // Zusätzliche Dokumente
        $DB_OrderDocs = $order->documents()->get();

        if($DB_OrderDocs){foreach($DB_OrderDocs as $DB_OrderDoc)
        {   $pdfName = $DB_OrderDoc->filename;
            $Doc_BelegNr = Order_Document_Attribute::where('fk_order_document_id', '=', $DB_OrderDoc->id)
            ->where('name', '=', 'beleg-nr')->first();
            if($Doc_BelegNr){$Doc_BelegNr = $Doc_BelegNr->value;}

            if(file_exists($pdfPath.$pdfName))
            {   $DocTypeDesc = $DB_OrderDoc->type()->first()->description;
                $tableRows[] = [ '<a href="'.route('tenant.orders.pdf.downloadDocument.file', [config('tenant.identifier'), $order->id, 'db_docs', $DB_OrderDoc->filename]).'">'.(($Doc_BelegNr)? $Doc_BelegNr : $DB_OrderDoc->filename ).'</a>', $DocTypeDesc ];
            }
        }}
    ...
    public function postOrder(Request $request)
    {
        ...
        if(property_exists($invoice_addr, 'email') && $invoice_addr->email != null) {
            try {
                ...
                $thisCC = 'schmoock@visc-media.de';
                $thisCCs = getenv('SHOP_MAIL_CC','schmoock@visc-media.de');

                if(config('tenant.identifier')=='frauenzimmer'){

                    $frauenzimmer_email='info@frauenzimmer-kirchhain.de';
                    $thisCC = $frauenzimmer_email;
                }
    ...
    public function indexRetours() {
        if(request()->ajax()) {
            return $this->dataTablesOrderData(Order::select('*')->with(['status', 'attributes', 'provider', 'provider.type',  'payment',  'shipment' ])
            ->whereIn('fk_order_status_id' , [Order_Status::getIdByKey('returned')]), 'retours');
    ...
    // Helpers
    public function floor3($val, $precision) {$precision = ($precision < 0) ? 0 : $precision; return round($val - (0.5 / pow(10, $precision)), $precision); }
}
