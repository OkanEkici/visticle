...
class ImportFC_modemai
{
    public function __construct(){}

    public function handle()
    {
        $tenant = Tenant::where('subdomain','=','modemai')->first();
        ...
        if(Setting::getFashionCloudApiKey() == null) { return; }
            ...
            //$fcArticleIds = Article::inRandomOrder()->where('fashioncloud_updated_at', '=', null)->where('active','=','1')->pluck('id')->toArray();
            ...
            try {
                ...
                $fcController->syncMultipleArticles($request, $fcArticleIds,true);
            }
            catch(Exception $ex) {
                if($synchro) {
